...
- `MIMO_WS_TUNNEL_TOKEN`、`MIMO_WS_NODE_RESPONSE_TIMEOUT_S`、`MIMO_WS_STREAM_CHUNK_TIMEOUT_S`、`MIMO_WS_MAX_PENDING` — （遗留）旧 WS 出站桥接相关；当前主线 SSH 部署不需要
...
- **SSH 反向隧道后端**：Claw 上的 `api-proxy.py` 经 `ssh -R` 把本机代理暴露到你指定目标机的 loopback 端口，Gateway 把该账号后端路由到 `http://<目标机>:<端口>`（普通 http 后端，直接转发）。详见下文「自动部署流程」。（旧的 WS 出站桥接 `gateway/ws_tunnel.py` + `ws-bridge.py` 仍保留但已不在主线部署路径。）
