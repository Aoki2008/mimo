...
- **WS 反向隧道后端**：可以像 `mimi3/mimo2api` 一样让 Claw 内的 `ws-bridge.py` 主动连回公网 `/ws`。面板新增后端时 `base_url` 填 `ws://公网域名:8088/ws` 或 `wss://公网域名/ws`，Gateway 会把 `/v1/chat/completions` / `/anthropic/v1/messages` 请求经该连接送进 Claw。
