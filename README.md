...
- `GATEWAY_DRAIN_TIMEOUT_S`、`GATEWAY_DEPLOY_DRAIN_GRACE_S`、`GATEWAY_MAINTENANCE_LOOP_INTERVAL_S`、`GATEWAY_MODEL_SYNC_INTERVAL_S` — Gateway 后端 drain、部署切换等待、维护循环和模型同步间隔（旧 `GATEWAY_ROTATION_LOOP_INTERVAL_S` 仍可作为兼容 fallback）
...
- **主动 chat 测活**：默认每 30s 探测当前 active 后端的 `/v1/chat/completions`；连续失败会进入 detection zone，以 10s 间隔快速复检。用户请求只写 metrics，不更新后端健康评级，避免客户端请求格式、取消流或侧边异常把健康后端降级。
